...
async function test_case() {
    ...
    assert.strictEqual(await driver.getCurrentUrl(), "http://localhost:3000/ShipmentQuote");
    ...
    const phoneNumberInput = await driver.findElement(By.name('PhoneNumberInput'));
    await phoneNumberInput.sendKeys('5555555555');

    await driver.executeScript("document.getElementById('MovingDateInput').value='2023-12-31'");

    const contactMethod = await driver.findElement(By.id('BothValue'));
    contactMethod.click()


    const additionalCommentsInput = await driver.findElement(By.name("AdditionalCommentsInput"));
    additionalCommentsInput.sendKeys("Additional Comments Test");

    // Pick up location section
    
    const PickUpAddressInput = await driver.findElement(By.name('PickUpAddressInput'));
    ...
    const PostalCodeInputP = await driver.findElement(By.name('PostalCodeInputP'));
    await PostalCodeInputP.sendKeys('J4K 4P3');

    const CountrySelectInputP = await driver.findElement(By.id('CountrySelectInputP'))
    CountrySelectInputP.click();
    await new Promise(resolve => setTimeout(resolve, 500));
    CountrySelectInputP.findElement(By.id('CanadaOptionP')).click();

    const BuildingTypeSelectInputP = await driver.findElement(By.id('BuildingTypeSelectInputP'))
    BuildingTypeSelectInputP.click();
    await new Promise(resolve => setTimeout(resolve, 500));
    BuildingTypeSelectInputP.findElement(By.id('HouseP')).click();

    const NumberOfRoomsInputP = await driver.findElement(By.name('NumberOfRoomsInputP'));
    ...
    const DropOffAddressInput = await driver.findElement(By.name('DropOffAddressInput'));
    ...
    const PostalCodeInputD = await driver.findElement(By.name('PostalCodeInputD'));
    await PostalCodeInputD.sendKeys('NY 10014');

    const CountrySelectInputD = await driver.findElement(By.id('CountrySelectInputD'))
    CountrySelectInputD.click();
    await new Promise(resolve => setTimeout(resolve, 500));
    CountrySelectInputD.findElement(By.id('USAOptionD')).click();

    const BuildingTypeSelectInputD = await driver.findElement(By.id('BuildingTypeSelectInputD'))
    BuildingTypeSelectInputD.click();
    await new Promise(resolve => setTimeout(resolve, 500));
    BuildingTypeSelectInputD.findElement(By.id('ApartmentD')).click();

    const NumberOfRoomsInputD = await driver.findElement(By.name('NumberOfRoomsInputD'));
    ...
    // shipment name 
    const ShipmentNameInput = await driver.findElement(By.name('ShipmentNameInput'));
